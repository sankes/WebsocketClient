package com.shankes.websocketclient.swipe.view;
...
public class SwipeItemLayout extends FrameLayout {
	...
	private static final int SENSITIVITY = 10;// 滑动触摸灵敏度,轻滑即可弹出右侧菜单(最小为2,越大越灵敏)
	...
	public boolean onSwipe(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			break;
		default:
			if ((mDownX - event.getX()) > (menuView.getWidth() / SENSITIVITY)) {
				// open
				smoothOpenMenu();
			} else {
				// close
	...
	private void swipe(int dis) {
		...
		menuView.layout(contentView.getWidth() - dis, menuView.getTop(), contentView.getWidth() + menuView.getWidth()
				- dis, menuView.getBottom());
	}
